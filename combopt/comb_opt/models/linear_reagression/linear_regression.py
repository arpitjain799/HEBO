...
from comb_opt.utils.onehot_utils import onehot_encode
...
class LinRegModel(ModelBase, torch.nn.Module):
    ...
    def __init__(self,
        ...
        super(LinRegModel, self).__init__(search_space, 1, dtype, device)
    ...
    def fit(self, x: torch.FloatTensor, y: torch.FloatTensor, **kwargs):
        ...
        self.y = y.clone().to(self.device, self.dtype)
        ...
        if self.estimator == 'bayes':
            self.alpha_mean, self.alpha_cov, self.a_post, self.b_post = self._train_bayes(x_train, self.y, self.a_prior,
                                                                                          self.b_prior)
        elif self.estimator == 'mle':
            self.alpha_mle = self._train_mle(x_train, self.y)

        elif self.estimator == 'horseshoe' or self.estimator == 'sparse_horseshoe':
            self.alpha_hs, _ = self._train_horseshoe(x_train, self.y, self.nb_gibbs, 0, 1, return_all=False)
        else:
