...
from comb_opt.utils.sampling import univariate_slice_sampling
from comb_opt.utils.training_utils import remove_repeating_samples
from comb_opt.utils.training_utils import subsample_training_data
...
class ComboGPModel(ModelBase):
    ...
    def fit(self, x: torch.FloatTensor, y: torch.FloatTensor):
        ...
        if len(y) > 1:
            ...
            self.y_std = torch.tensor(1.0, dtype=self.dtype, device=self.device) if self.y_std < 1e-6 else self.y_std
            self.y = (self.y - self.y_mean) / self.y_std

        self.output_min = torch.min(self.y)
        self.output_max = torch.max(self.y)

        if not self.is_initialised:
            self._initialise(self.y)
    ...
    def predict(self, x: torch.FloatTensor, **kwargs) -> (torch.FloatTensor, torch.FloatTensor):
        ...
        mu = (self.y_std * pred_mean + self.y_mean).view(-1, self.num_out)
    ...
    def slice_noisevar(self) -> torch.FloatTensor:
        ...
        log_likelihood_noise = np.maximum(log_likelihood_noise, np.log(self.n_lb))
    ...
    def gram_mat_update(self, mean: float, log_amp: float, log_beta: torch.FloatTensor, log_likelihood_noise: float):
        ...
        self.mean_vec = self.y.view(-1, 1) - self.mean(self.x).view(-1, 1)
    ...
    def negative_log_likelihood(self, constmean: float, log_amp: float, log_beta: torch.FloatTensor,
        ...
        nll = 0.5 * torch.sum(mean_vec_sol ** 2) + torch.sum(
            torch.log(torch.diag(self.cholesky))) + 0.5 * self.y.size(0) * np.log(2 * np.pi)
...
class ComboEnsembleGPModel(EnsembleModelBase):
    ...
    def __init__(self,
        ...
        self.combo_gp = ComboGPModel(search_space, fourier_freq_list, fourier_basis_list, n_vertices,
                                     adjacency_mat_list, n_lb, n_burn, n_burn_init,
