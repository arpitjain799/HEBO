...
class GpSskLsAcqOptim(BoBase):

    @property
    def name(self) -> str:
        if self.use_tr:
            name = f'GP (SSK) - Tr-based LS acq optim'
        else:
            name = f'GP (SSK) - LS acq optim'
        return name
    ...
    def __init__(self,
        ...
        super(GpSskLsAcqOptim, self).__init__(search_space, n_init, model, acq_func, acq_optim, tr_manager,
                                              dtype, device)
