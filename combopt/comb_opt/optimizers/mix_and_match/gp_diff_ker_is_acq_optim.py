...
class GpDiffusionTrLsAcqOptim(BoBase):
    ...
    def name(self) -> str:
        if self.use_tr:
            name = f'GP (Diffusion) - Tr-based IS acq optim'
        else:
            name = f'GP (Diffusion) - IS acq optim'
        return name
    ...
    def __init__(self,
        ...
        tr_acq_func = SingleObjAcqExpectation(acq_factory(acq_func_name=tr_restart_acq_name))
