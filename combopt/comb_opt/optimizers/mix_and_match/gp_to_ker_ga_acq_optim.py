...
class GpToGaAcqOptim(BoBase):

    @property
    def name(self) -> str:
        if self.use_tr:
            name = f'GP (TO) - Tr-based GA acq optim'
        else:
            name = f'GP (TO) - GA acq optim'
        return name
    ...
    def __init__(self,
        ...
        super(GpToGaAcqOptim, self).__init__(search_space, n_init, model, acq_func, acq_optim, tr_manager, dtype,
                                             device)
