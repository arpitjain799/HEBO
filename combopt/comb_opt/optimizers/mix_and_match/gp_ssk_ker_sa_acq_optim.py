...
class GpSskSaAcqOptim(BoBase):
    ...
    def name(self) -> str:
        if self.use_tr:
            name = f'GP (SSK) - Tr-based SA acq optim'
        else:
            name = f'GP (SSK) - SA acq optim'
        return name
    ...
    def __init__(self,
        ...
        super(GpSskSaAcqOptim, self).__init__(search_space, n_init, model, acq_func, acq_optim, tr_manager, dtype,
                                              device)
