...
class GpOIsAcqOptim(BoBase):

    @property
    def name(self) -> str:
        if self.use_tr:
            name = f'GP (O) - Tr-based IS acq optim'
        else:
            name = f'GP (O) - IS acq optim'
        return name
    ...
    def __init__(self,
        ...
        super(GpOIsAcqOptim, self).__init__(search_space, n_init, model, acq_func, acq_optim, tr_manager, dtype, device)
