...
class GpOSaAcqOptim(BoBase):

    @property
    def name(self) -> str:
        if self.use_tr:
            name = f'GP (O) - Tr-based SA acq optim'
        else:
            name = f'GP (O) - SA acq optim'
        return name
    ...
    def __init__(self,
        ...
        super(GpOSaAcqOptim, self).__init__(search_space, n_init, model, acq_func, acq_optim, tr_manager, dtype,
                                            device)
